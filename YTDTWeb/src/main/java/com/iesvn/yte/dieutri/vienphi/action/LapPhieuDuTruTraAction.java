 ...
 public class LapPhieuDuTruTraAction
   implements Serializable
 {
   ...
   public void huyPhieuDT()
   {
     ...
     String returnKQ = ctPhieuDTDelegate.huyPhieuDuTruTra_New(this.phieuDuTru.getPhieudtMa());
   ...
   public void Caculation(List<CtPhieuDt> ctpdtlist)
   {
     ...
     if (ctpdtlist != null)
     {
       ...
       {
         System.out.println("ctpdt.getCtdtDongia(): " + ctpdt.getCtdtDongia());
         System.out.println("ctpdt.getCtdtTra(): " + ctpdt.getCtdtTra());
         dTongTien = Double.valueOf(dTongTien.doubleValue() + ctpdt.getCtdtDongia().doubleValue() * ctpdt.getCtdtTra().doubleValue());
       }
     ...
     throws Exception
   {
     ...
     try
     {
       ...
       this.phieuDuTru.setPhieudtPhanBiet(Integer.valueOf(2));
       ...
       this.maFinish = ctPhieuDelegate.capNhatPhieuDuTruTra(this.listCtkq, this.listTNT, this.phieuDuTru, maphieu);
   ...
   {
     this.reportTypeVP = "B3125_2_KhoaPhongTraTheoPDT";
     log.info("Vao Method XuatReport B3125_2_KhoaPhongTraTheoPDT");
     if (this.phieuDuTru.getPhieudtMa() != null) {
       try
       {
         ...
         xuatReport.XuatReportPhieuDuTruKhoaPhong(log, this.phieuDuTru, this.index, loaiThuoc);
     ...
     throws Exception
   {
     ...
     if ((this.phieuDuTru.getPhieudtMa() == null) || (this.phieuDuTru.getPhieudtMa().equals(""))) {
       try
       {
         ...
         Date tuNgay = null;
         Date denNgay = null;
         if ((this.ngayXuat != null) && (!this.ngayXuat.equals("")) && (this.ngayXuat != null) && (!this.ngayXuat.equals(""))) {
           try
           {
           ...
           {
             System.out.println("Loi khi chuyen doi dinh dang ngay" + e);
           }
         }
         ...
         if (tntTemps != null)
         {
           for (int i = 0; i < tntTemps.size(); i++)
           {
             ...
             if (tntTemp.getThuocnoitruDongia().doubleValue() == 0.0D) {
         ...
         {
           log.info("***tnt_temp.size: " + listThuocNoiTru.size());
           for (ThuocNoiTru tnt : listThuocNoiTru)
           {
             CtPhieuDt ctdt = changetoData(tnt, this.listCtkq);
             SetInforUtil.setInforIfNullForCTPhieuDuTru(ctdt);
             this.listCtkq.add(ctdt);
             log.info("listCtkq.size(): " + this.listCtkq.size());
           }
           Caculation(this.listCtkq);
         }
       }
     ...
     throws Exception
   {
     ...
     if (list.size() > 0) {
       ...
       {
         CtPhieuDt temp = (CtPhieuDt)BeanUtils.cloneBean(list.get(i));
         if (temp.getDmthuocMaso().getDmthuocMa().equals(tnt.getThuocnoitruMathuoc().getDmthuocMa()))
         {
           list.remove(i);
           temp.setCtdtTra(Double.valueOf((tnt.getThuocnoitruTra() == null ? 0.0D : tnt.getThuocnoitruTra().doubleValue()) + (temp.getCtdtTra() == null ? 0.0D : temp.getCtdtTra().doubleValue())));
           result = temp;
           haveinlist = true;
           break;
         }
     ...
     throws Exception
   {
     try
     {
       ...
       this.phieuDuTru = ctPhieuDTDelegate.findByPhieuDuTruPhanBiet(this.phieuDuTru.getPhieudtMa(), Integer.valueOf(2));
       ...
       {
         List<CtPhieuDt> ctpdt_tmp = ctPhieuDTDelegate.findByPhieuDuTruMa(this.phieuDuTru.getPhieudtMa());
         log.info("Co thong tin phieu du tru" + this.phieuDuTru);
         ...
         this.dmDoiTuongTen = this.phieuDuTru.getDmdoituongMaso().getDmdoituongTen();
 ...
 * Qualified Name:     com.iesvn.yte.dieutri.vienphi.action.LapPhieuDuTruTraAction
