...
dojo.declare(
	...
	null,
	{
		...
		_myfunction: function(){
			...
			//if(countRecord(this.name,"") == false){
			//			listCatalog_search(this);
			//}
			//alert(1);
			var doSearch = false;
		...
		_getQueryString: function(/*String*/ text){
			...
			/*
			//---my Advance code Hy Tien--- 
			var mySearchText_arr = text.split("");
			var mySearchText_use = "*";
			if(text != ""){
				if(mySearchText_arr[0].isUpperCase()){
					for(var i=0; i<mySearchText_arr.length; i++){
						mySearchText_use += " " + mySearchText_arr[i] + "*";
					}	
				}
				else{
					mySearchText_use += " " + text + "*";
				}
			}
			return dojo.string.substitute(this.queryExpr, [mySearchText_use]);
