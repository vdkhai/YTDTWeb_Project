    ...
    if (window.google && google.gears) {
        ...
        try {
        	//db = google.gears.factory.create('beta.database');
            	...
            	setAttrForCombobox(prefix_component + "NGUOINHAP_MA","DT_DM_NHAN_VIEN_span","DT_DM_NHAN_VIEN","getDtDmNhanVien_Duoc()","","","");
...
function checkAddChiTiet(){
	...
	if (result){
		...
		if (parseInt(tra) > parseInt(soluong)) {
			alert("Số lượng trả lại phải nhỏ hơn hoặc bằng số lượng đang có");
			document.getElementById(prefix_component + "__xuat").value = "";			
			document.getElementById(prefix_component + "__xuat").focus();
			result = false;			
		}
