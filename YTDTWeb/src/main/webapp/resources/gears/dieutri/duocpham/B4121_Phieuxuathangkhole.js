function init() {
	...
	if (window.google && google.gears) {
		// try {
            ...
            setAttrForCombobox(prefix_component + "DM_KHOA_XUAT","DM_KHO_span2","DM_KHOA__2","getDmKhoa_Ct(\""+ tenCt +"\")","","","");
...
//			lockUnlockControl(document.forms[0], 1);
//		}
//		else lockUnlockControl(document.forms[0], 0);
//		
...
function tinhTien() {
	/*
	 * var countCt = document.getElementById(prefix_component +
	 * "__count").value; var temp = 0; for (var i = 0; i < countCt; i++) { var
	 * tt = document.getElementById(prefix_component + "tableXuatKho:" + i +
	 * ":colTT").innerHTML; temp = temp + parseInt(tt); //alert("tong tien: " +
	 * temp); } document.getElementById(prefix_component + "__tongtien").value =
	 * temp;
	 */
}
  	...
  	try {
		...
		xmlHttp.onreadystatechange = function() {
			if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
				...
				if (data.list.record) {
							while (data.list.record[i] != null) {      // truong
																		// hop
																		// nay
																		// co'
																		// >=2
				...
				if (data.list.record) {
					while (data.list.record[i] != null) {      // truong hop
																// nay co' >=2
																// record
...
/*
 * function onfocusTonkho() { if (document.getElementById(prefix_component +
 * "__tonkhoma_hid").value == "") { dmtFocus(); } else {
